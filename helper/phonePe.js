...
module.exports.Pay = async (userId, transaction_id, mobileNumber, amount) => {
  try {
    ...
    console.error("Pay options error:", error);
    throw new Error(`Error occured inside phonepe: Pay. Error: ${error}`);
  }
};
...
module.exports.CheckStatus = async (transaction_id) => {
  try {
    ...
    console.error("CheckStatus options error:", error);
    throw new Error(
      `Error occured inside phonepe: CheckStatus. Error: ${error}`
    );
  }
};
